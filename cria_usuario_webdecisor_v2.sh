...
LOG_FILE="cria_${usuario_fixo}.log"
echo "########################################################" | tee -a "$LOG_FILE"
...
if [ $? -ne 0 ]; then
    echo "[ERRO] Falha ao executar comandos SQL para criar/configurar o usuário ${usuario_fixo}." | tee -a "$LOG_FILE"
...
$SUDO_CMD sed -i.bak "/# Configurado por script ${usuario_fixo}/d" "$PG_HBA_CONF"
$SUDO_CMD sed -i "/${usuario_fixo}/d" "$PG_HBA_CONF"
...
if ! PGPASSWORD="$PGPASSWORD_TEST" psql -X -h "$END_SERVIDOR" -U "$usuario_fixo" -d "$CHINCHILA_DS_DATABASENAME" -c "SELECT * FROM integracao_webdecisor.v_extracaocliente LIMIT 1;" > /dev/null 2> erro_teste1.log; then
...
if ! PGPASSWORD="$PGPASSWORD_TEST" psql -X -h "$END_SERVIDOR" -U "$usuario_fixo" -d "$CHINCHILA_DS_DATABASENAME" -c "SELECT * FROM unidademedida LIMIT 1;" > /dev/null 2> erro_teste2.log; then
...
if $TEST_PASSED; then
  ...
  echo "Senha: $senha_novo_usuario" | tee -a "$LOG_FILE"
  ...
  if [[ "$resposta" =~ ^[sS]$ ]]; then
    ...
    psql -X -h "$END_SERVIDOR" -U postgres -d "$CHINCHILA_DS_DATABASENAME" -c "DROP ROLE IF EXISTS \"${usuario_fixo}\";"
    echo "[INFO] Removendo entrada de ${usuario_fixo} do pg_hba.conf (se adicionada)..." | tee -a "$LOG_FILE"
    if [[ -n "$SUDO_CMD" ]]; then
        $SUDO_CMD sed -i.bak "/# Configurado por script ${usuario_fixo}/d" "$PG_HBA_CONF"
        $SUDO_CMD sed -i "/${usuario_fixo}/d" "$PG_HBA_CONF"
    else
        sed -i.bak "/# Configurado por script ${usuario_fixo}/d" "$PG_HBA_CONF"
        sed -i "/${usuario_fixo}/d" "$PG_HBA_CONF"
    fi
