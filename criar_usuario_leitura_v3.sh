...
if PGPASSWORD="$senha" psql -X -h "$END_SERVIDOR" -U "$usuario" -d "$CHINCHILA_DS_DATABASENAME" -c "SELECT * FROM UNIDADENEGOCIO LIMIT 1;" > /dev/null 2> erro.log; then
  ...
  echo "IP Servidor Local: $END_SERVIDOR  -  (Caso a conexão à base seja feita no mesmo ambiente de rede) " | tee -a "$LOG_FILE "
  echo "IP Servidor Público: $IP_PUBLICO  -  (Caso a conexão à base seja feita de outro ambiente de rede) " | tee -a "$LOG_FILE  "
